...
Crafty.c("topHUD",
{
	...
	topHUD : function(x_offset, y_offset, character) {
		...
		var textFontAttr = {  family: 'Cursive', 'font-style':'italic' };
		this.curNameTextEntity.text(character.characterName + " Level:&nbsp;" + character.level).textColor('#000000', 1).
		css("textAlign", "left").textFont(textFontAttr);
		// names of bench
		for (var i = 0; i < benchCharacters.length; i++) {	
			var benchNameTextEntityKey = i + character.targetPlayer.targetPlayerType;
		
			if (this.benchNameTextEntities[benchNameTextEntityKey] == null) {
				this.benchNameTextEntities[benchNameTextEntityKey] = Crafty.e("2D, DOM, Text").attr(
						{ 
			...
			this.benchNameTextEntities[benchNameTextEntityKey].text(benchCharacters[i].character.characterName + "&nbsp;Level:&nbsp;" + benchCharacters[i].character.level).
			textColor('#000000', 1).css("textAlign", "left").textFont(textFontAttr);
		}
